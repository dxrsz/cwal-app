...
<div class="relative overflow-hidden rounded-lg px-4 py-3 {containerColor.bg}" aria-label={label}>
  <div class="absolute inset-0 opacity-15">
    <div class="h-full w-full {containerColor.accent} animate-pulse"></div>
  </div>
  <div class="relative flex items-center gap-6">
    <p class="text-sm font-medium {containerColor.text} truncate flex-1 min-w-0">{label}</p>
    <div class="flex items-center gap-4 {containerColor.text}">
      <div class="flex items-baseline gap-1">
        ...
        <span class="text-sm font-semibold leading-none tabular-nums">{apiRequests}</span>
      </div>
      <div class="flex items-baseline gap-1">
        <span class="text-[10px] uppercase tracking-wide opacity-70">Replays</span>
        <span class="text-sm font-semibold leading-none tabular-nums">{replaysDownloaded}</span>
      </div>
    </div>
  </div>
</div>
