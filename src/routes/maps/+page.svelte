<script lang="ts">
  ...
  let currentSeason: Awaited<ReturnType<typeof GravaticBooster.prototype.currentSeason>> = $state(0);
  let season: string | undefined = $state(undefined);
  let maps: Awaited<ReturnType<typeof GravaticBooster.prototype.maps>> = $state([]);

  const seasons = $derived(Array(currentSeason).keys().map(n => n+1).toArray().reverse());
...
<Select.Root type="single" name="season" bind:value={season}>
  <Select.Trigger class="w-[180px]">{season != null ? `Season ${season}` : "Season"}</Select.Trigger>
  <Select.Content>
    <Select.Group>
...
{#each maps as map}
  ...
  <Table.Row>
        ...
        <Table.Cell>{new Date(map.modified_epoch).toLocaleDateString()}</Table.Cell>
        <Table.Cell class="text-right"><a href="{map.url}">Download</a></Table.Cell>
      </Table.Row>
  {/if}
{/each}
</Table.Body>
